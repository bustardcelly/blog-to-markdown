...
class MarkdownTask
  constructor: (@config) ->
    ...
    @site = new Site(@config, @buildPosts(), new Layout(@config.layouts.post, @config.context))
  ...
  createPosts: ->
    generatesHtml = new GeneratesHtml(@wrapper, new Layout(@config.layouts.post, @config.context), @site)
    _(@site.posts).each (post) =>
      html = generatesHtml.generate(post)
      @writesFile.write(html, post.htmlPath())

  createPages: ->
    generatesHtml = new GeneratesHtml(@wrapper, new Layout(@config.layouts.page, @config.context), @site)
  ...
  createIndex: ->
    ...
    html = new GeneratesHtml(@wrapper, new Layout(@config.layouts.index, @config.context), @site).generate(post)
    @writesFile.write(html, @config.paths.index)

  createArchive: ->
    html = new GeneratesHtml(@wrapper, new Layout(@config.layouts.archive, @config.context), @site).generate()
